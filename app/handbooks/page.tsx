...
export const metadata: Metadata = {
  ...
  description: 'Explore beginner-friendly visual handbooks for RAG, LangChain, Docker, GitHub Copilot and Forward Deployed Engineering with practical workflows and scenarios.',
  alternates: { canonical: '/handbooks/' },
  openGraph: {title:'Visual Technology Handbooks | MaanavaN Bytes',description:'Connected, beginner-friendly handbooks for RAG, LangChain, Docker, GitHub Copilot and Forward Deployed Engineering.',url:'/handbooks/',type:'website'},
};
...
export default function HandbooksPage() {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/>
    ...
    <section className="handbooks-library container-custom"><header><div><p className="eyebrow dark">PUBLISHED HANDBOOKS</p><h2>Choose the path that fits your next step.</h2></div><div className="handbook-search-visual"><Search/><span>RAG · LangChain · Docker · Copilot · Forward deployment</span></div></header><div className="handbooks-grid">{handbooks.map((handbook)=>{const href=handbook.slug==='github-copilot'?'/software-engineering/':handbook.slug==='docker'?'/cloud-devops/':`/${handbook.slug}/`;return <Link className={`handbook-tile handbook-tone-${handbook.tone}`} href={href} key={handbook.slug}><div className="handbook-tile-top"><span>VISUAL GUIDE</span><small>{handbook.audience}</small></div><div className="handbook-tile-icon"><BookOpen/></div><h2>{handbook.title}</h2><p>{handbook.description}</p><footer><span>Visual learning path</span><b>Start learning <ArrowRight/></b></footer></Link>})}</div></section>
  </>;
}
