...
export default function HandbooksPage() {
  return <>
    ...
    <section className="handbooks-library container-custom"><header><div><p className="eyebrow dark">EXPLORE BY TOPIC</p><h2>Find the handbook that fits your next step.</h2></div><div className="handbook-search-visual"><Search/><span>AI, engineering, careers and tools</span></div></header><div className="handbooks-grid">{handbooks.map((handbook,index)=><Link className={`handbook-tile handbook-tone-${handbook.tone}`} href={handbook.slug === 'github-copilot' ? '/software-engineering/' : handbook.slug === 'forward-deployed-engineer' ? '/forward-deployed-engineer/' : `/handbooks/${handbook.slug}/`} key={handbook.slug}><div className="handbook-tile-top"><span>{String(index+1).padStart(2,'0')}</span><small>{handbook.audience}</small></div><div className="handbook-tile-icon"><BookOpen/></div><h2>{handbook.title}</h2><p>{handbook.description}</p><footer><span>{handbook.chapters.length} chapters</span><b>{handbook.status === 'available' ? 'Start learning' : 'Preview path'} <ArrowRight/></b></footer></Link>)}</div></section>
  </>;
}
