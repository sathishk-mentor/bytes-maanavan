...
import { ArrowRight, BookOpen, BriefcaseBusiness, Clock3, Code2, Database, GraduationCap, Search, ShieldCheck, Sparkles, Users } from 'lucide-react';
...
export default function HomePage() {
  ...
  return <>
    ...
    <section id="career-paths" className="career-section"><div className="container-custom"><div className="section-heading"><p className="eyebrow dark">Learn for your next role</p><h2>Choose a career direction, then follow the right concepts.</h2><p>Move from scattered tutorials to a focused sequence based on the work you want to do.</p></div><div className="career-grid">
      <Link href="/genai/"><Sparkles/><div><h3>AI Engineer</h3><p>GenAI · RAG · Agents</p></div><ArrowRight/></Link>
      <Link href="/software-engineering/"><Code2/><div><h3>Full-Stack Developer</h3><p>Software · APIs · Cloud</p></div><ArrowRight/></Link>
      <Link href="/data-engineering/"><Database/><div><h3>Data Engineer</h3><p>Pipelines · Warehouses · Streaming</p></div><ArrowRight/></Link>
      <Link href="/cybersecurity/"><ShieldCheck/><div><h3>Cybersecurity Analyst</h3><p>Threats · IAM · AI Security</p></div><ArrowRight/></Link>
    </div></div></section>
    <section id="interview-prep" className="interview-section"><div className="container-custom interview-inner"><div><p className="eyebrow">Interview-ready learning</p><h2>Understand it well enough to explain it.</h2><p>Every upgraded Byte will include a concise interview answer, follow-up questions and a practical example—so learning translates into confident conversation.</p><Link className="primary-cta" href="/case-studies/">Explore practical case studies <ArrowRight/></Link></div><div className="interview-card"><span>INTERVIEW BIT</span><h3>“Can you explain this concept using a real system?”</h3><p>Start with the problem. Explain the components. Describe the data flow. Finish with one trade-off.</p><small>Simple framework · Stronger answer</small></div></div></section>
