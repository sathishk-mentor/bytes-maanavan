...
export function AccordionTableOfContents({headings}:{headings:Heading[];byteSlug:string}) {
  ...
  useEffect(()=>{
    const tanglish=document.documentElement.dataset.readingMode==='tanglish';
    const targets=sections.map((section)=>document.getElementById(`${tanglish?'ta-':''}${section.id}`)).filter(Boolean) as HTMLElement[];
    ...
    const refresh=()=>{ observer.disconnect(); const useTanglish=document.documentElement.dataset.readingMode==='tanglish'; sections.map((section)=>document.getElementById(`${useTanglish?'ta-':''}${section.id}`)).filter(Boolean).forEach((target)=>observer.observe(target as Element)); };
    window.addEventListener('reading-mode-change',refresh);
    return()=>{observer.disconnect();window.removeEventListener('reading-mode-change',refresh)};
  },[sections]);
  const go=(id:string)=>{setActiveId(id);const prefix=document.documentElement.dataset.readingMode==='tanglish'?'ta-':'';document.getElementById(`${prefix}${id}`)?.scrollIntoView({behavior:'smooth',block:'start'});setOpen(false)};
