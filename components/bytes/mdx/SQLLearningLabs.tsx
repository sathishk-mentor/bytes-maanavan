...
function QueryConsole({query,children}:{query:string;children:React.ReactNode}){
  const [draft,setDraft]=useState(query);const [status,setStatus]=useState<'ready'|'running'|'ran'|'blocked'|'empty'>('ready');
  useEffect(()=>{setDraft(query);setStatus('ready')},[query]);
  const run=()=>{const clean=draft.trim();if(!clean){setStatus('empty');return}if(/\b(DELETE|DROP|TRUNCATE|UPDATE|INSERT)\b/i.test(clean)){setStatus('blocked');return}setStatus('running');window.setTimeout(()=>setStatus('ran'),520)};
  const message=status==='running'?'Executing the query against the guided sample dataset…':status==='ran'?'Query completed. Inspect the returned rows and learning insight.':status==='blocked'?'Write operation blocked. This lab permits read-only SELECT queries.':status==='empty'?'Add a SQL statement before running the query.':'Edit the SQL, then select Run query to generate the result.';
  return <section className={`sql-query-console status-${status}`}>
    <header><span><TerminalSquare/><small>INTERACTIVE SQL QUERY LAB</small><strong>Write → Run → Inspect → Learn</strong></span><div><button onClick={()=>{setDraft(query);setStatus('ready')}}><RotateCcw/>Reset SQL</button><button className="sql-run" onClick={run} disabled={status==='running'}><Play/>{status==='running'?'Running…':'Run query'}</button></div></header>
    <div className="sql-workbench"><label><span className="sql-pane-label">QUERY EDITOR <em>SQL</em></span><textarea value={draft} onChange={event=>{setDraft(event.target.value);setStatus('ready')}} spellCheck={false}/></label><aside><span className="sql-pane-label">SAMPLE DATABASE <em>READ ONLY</em></span><b>Connected</b><strong>maanavan_learning</strong><ul><li><i/>products</li><li><i/>orders</li><li><i/>customers</li></ul></aside></div>
    <footer aria-live="polite"><i/>{message}</footer>
    {status==='running'&&<div className="sql-running" aria-label="Query is running"><span/><span/><span/></div>}
    {status==='ran'&&<div className="sql-output"><header><span><i/>QUERY RESULT</span><small>Completed in 42 ms · Guided dataset</small></header><div>{children}</div></div>}
  </section>
}
...
export function SQLAgentLab(){
  ...
  return <Shell eyebrow="BYTE 05 · SAFE TEXT-TO-SQL" title="Watch the agent decide before execution" onReset={()=>setQuestion('ambiguous')}><div className="sql-tabs three"><button aria-pressed={question==='ambiguous'} onClick={()=>setQuestion('ambiguous')}>Ambiguous metric</button><button aria-pressed={question==='clear'} onClick={()=>setQuestion('clear')}>Clear request</button><button aria-pressed={question==='unsafe'} onClick={()=>setQuestion('unsafe')}>Unsafe write</button></div><div className="sql-agent-flow"><span>Question</span><i>→</i><span>Definitions</span><i>→</i><span>Generate</span><i>→</i><span>Validate</span><i>→</i><b>{state[0]}</b></div><QueryConsole query={query}><Result title={state[0]}>{state[1]}</Result></QueryConsole></Shell>;
}
