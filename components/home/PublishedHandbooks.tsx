...
function HandbookCard({ handbook, compact = false }: { handbook: Handbook; compact?: boolean }) {
  ...
  if (compact) return <article className={`home-handbook-cover home-handbook-${tone}`}>
    ...
    <div className="home-handbook-visual" aria-hidden="true"><span className="home-handbook-grid"/><div className="home-handbook-visual-top"><span><Icon/>HANDBOOK</span><b>{handbook.chapters.length} BYTES</b></div><HandbookScene slug={handbook.slug}/><small>{handbook.label}</small></div>
...
export function PublishedHandbooks({ handbooks, variant = 'home' }: { handbooks: Handbook[]; variant?: 'home' | 'library' }) {
  ...
  return <section className={`published-handbooks handbook-collection handbook-collection-${variant}`}><div className="container-custom">
    ...
    {variant === 'home' ? <div className="handbook-library-layout"><aside className="handbook-category-sidebar" aria-label="Handbook categories"><strong>Categories</strong>{filters.map((item) => <button className={filter === item.value ? 'active' : ''} onClick={() => setFilter(item.value)} key={item.value}><span>{item.label}</span><b>{item.value === 'all' ? handbooks.length : handbooks.filter((handbook) => handbook.category === item.value).length}</b></button>)}</aside><div className="handbook-library-results"><div className="handbook-result-summary"><span><BookOpen/>Showing <b>{results.length}</b> matching {results.length === 1 ? 'handbook' : 'handbooks'}</span>{(query || filter !== 'all') && <button onClick={() => { setQuery(''); setFilter('all'); }}>Clear filters</button>}</div>
