  ...
  return <header className="site-header sticky top-0 z-50">
    ...
    <div className="header-main container-custom">
      ...
      <nav className="desktop-nav" aria-label="Main navigation">
        ...
        <div className="nav-dropdown handbook-dropdown" onMouseEnter={()=>setTracksOpen(true)} onMouseLeave={()=>setTracksOpen(false)}><button onClick={()=>setTracksOpen(!tracksOpen)} aria-expanded={tracksOpen}>Handbook <ChevronDown/></button><div className={`track-menu handbook-menu ${tracksOpen?'is-open':''}`}><header><span><Code2/></span><div><strong>The GitHub Copilot Handbook</strong><small>Five practical chapters · Reviewed by Sathish Kumar</small></div><Link href="/software-engineering/">View all <ArrowRight/></Link></header>{chapters.map(([number,title,slug])=><Link key={slug} href={`/software-engineering/${slug}/`} onClick={()=>setTracksOpen(false)}><i>{number}</i><strong>{title}</strong><ArrowRight/></Link>)}</div></div>
