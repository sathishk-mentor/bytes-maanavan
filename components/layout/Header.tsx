...
export function Header() {
  ...
  const categories = [{ slug: 'software-engineering', title: 'GitHub Copilot Handbook', description: 'Five practical chapters for AI-assisted software development.' }];
  return <header className="site-header sticky top-0 z-50">
    ...
    <div className="header-main container-custom">
      ...
      <nav className="desktop-nav" aria-label="Main navigation">
        ...
        <div className="nav-dropdown"><button onClick={()=>setTracksOpen(!tracksOpen)} aria-expanded={tracksOpen}>Handbook <ChevronDown/></button>{tracksOpen&&<div className="track-menu">{categories.map(c=><Link key={c.slug} href={`/${c.slug}/`} onClick={()=>setTracksOpen(false)}><strong>{c.title}</strong><small>{c.description}</small></Link>)}</div>}</div>
    ...
    {open&&<nav className="mobile-nav container-custom" aria-label="Mobile navigation"><Link href="/" onClick={()=>setOpen(false)}>Bytes Home</Link><p>Handbook</p>{categories.map(c=><Link className="mobile-track" key={c.slug} href={`/${c.slug}/`} onClick={()=>setOpen(false)}>{c.title}</Link>)}<Link href="/#interview-prep" onClick={()=>setOpen(false)}>Interview Prep</Link><a href={courseLibrary}>Course Library</a><a href={membership}>All-Access Membership</a><a className="mobile-course-cta" href={courseLibrary}>Explore Course Library <ArrowRight/></a></nav>}
  </header>;
}
