...
<KnowledgeCheck question={"Which behaviour most clearly makes a system an AI agent?"} options={["It only answers a question","It chooses authorised actions toward a goal and checks the result","It stores every user message"]} answer={1} explanation={"An agent works toward a bounded outcome by selecting actions, using tools and checking progress."} />
