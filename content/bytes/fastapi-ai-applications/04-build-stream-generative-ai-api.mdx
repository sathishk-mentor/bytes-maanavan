...
summary: "Build a Generative AI API with FastAPI, connect retrieval and an LLM, stream grounded response events and return validated citations."
...
<KnowledgeCheck question={"What must a streaming AI API define clearly?"} options={["Only the first token","Event format, completion, errors and cancellation behaviour","A different port for every token"]} answer={1} explanation={"A stream is a contract: clients must recognise data, evidence, completion and failure events."} />
