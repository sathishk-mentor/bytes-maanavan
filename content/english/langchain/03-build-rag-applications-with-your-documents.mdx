...
<KnowledgeCheck question={"Why retrieve selected chunks instead of sending every document on each question?"} options={["To reduce relevance","To supply focused evidence with lower noise and cost","To avoid source tracking"]} answer={1} explanation={"Focused retrieval gives the model the most useful evidence without repeatedly sending the entire collection."} />
